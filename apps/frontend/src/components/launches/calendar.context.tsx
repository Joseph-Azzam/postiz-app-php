...
export const CalendarWeekProvider: FC<{
  ...
  const { isLoading } = swr;
  // Stable empty fallback so useEffect([posts]) doesn't run every render when loading (avoids "Maximum update depth").
  const emptyResult = useMemo(() => ({ posts: [] as Post[], comments: [] as Array<{ date: string; total: number }> }), []);
  const { posts, comments } = swr?.data ?? emptyResult;
  ...
  useEffect(() => {
    if (swr?.data?.posts) {
      setInternalData(swr.data.posts);
    }
  }, [swr.data]);
