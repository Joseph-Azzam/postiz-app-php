...
export const AddProviderComponent: FC<{
  ...
  const { update, social: socialProp, article: articleProp, onboarding } = props;
    ...
    (
      ...
      async () => {
        ...
        const gotoIntegration = async (externalUrl?: string) => {
          ...
          const { url, err, message } = data;
          if (err) {
            toaster.show(
              message || t('could_not_connect_to_platform', 'Could not connect to the platform'),
              'warning'
            );
            return;
          }
        ...
        // Postiz Laravel: check customFields before isExternal so Bluesky (service/identifier/password form) is used instead of URL modal
        if (customFields) {
          modal.openModal({
