...
export const ContinueIntegration: FC<{
  ...
  useEffect(() => {
    (async () => {
      ...
      // X (Twitter): OAuth 1.0a callback uses oauth_token + oauth_verifier; OAuth 2.0 uses code + state.
      // Only override when we have OAuth 1.0a params so we don't overwrite OAuth 2.0 code/state with empty.
      if (provider === 'x' && searchParams.oauth_token != null && searchParams.oauth_verifier != null) {
